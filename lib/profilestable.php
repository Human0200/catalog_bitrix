...
class ProfilesTable extends Main\Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			new Entity\StringField('neworder', [
				'title' => Loc::getMessage('SP_CI_PROFILES_ENTITY_NEWORDER_FIELD'),
			]),
	...
	protected static function checkFilterConformity($has_conformity, $condition, $value) {
		if ($has_conformity) {
			...
			// Delete last empty value
			if (count($condition['value']) > 1 && trim($condition['value'][count($condition['value'])-1]) == '') {
				unset($condition['value'][count($condition['value'])-1]);
			}
	...
	/**
	 * Search profile by profile filter
	 *
	 * @param $order_data
	 *
	 * @return void
	 */
	public static function findProfile($order_data, $deal=[]) {
		$fields = false;
		$list = self::getList([
			'filter' => ['active' => 'Y'],
		]);
		if (!empty($list)) {
			foreach ($list as $profile) {
				...
				if (self::checkProfile($order_data, $deal, $profile)) {
					$fields = $profile;
					break;
				}
			}
	...
	/**
	 * Search profile by deal category and stage
	 *
	 * @param $order_data
	 * @param $deal
	 *
	 * @return void
	 */
	public static function getProfileByOrder($order_data, $deal) {
		$result = false;
		$list = self::getList([
			'filter' => ['active' => 'Y'],
		]);
		if (!empty($list)) {
			foreach ($list as $profile) {
				// Find by category_id and stage
				$status_id = self::getStatusByStage($deal['STAGE_ID'], $profile);
				// Check by filter without status
				if ($status_id) {
					if (self::checkProfile($order_data, $deal, $profile, ['status'])) {
						$result = $profile;
						break;
					}
				}
	...
	public static function getStatusByStage($stage_id, $profile) {
		$status_id = false;
		$comp_table = $profile['statuses']['comp_table'];
		foreach ($comp_table as $ct_status => $ct_item) {
			if (in_array($stage_id, $ct_item['stages'])) {
				$status_id = $ct_status;
			}
		}
		return $status_id;
	...
	public static function checkProfile($order_data, $deal, $profile, $excludes=[]) {
		$has_conformity = true;
		// Check deal category
		if (!empty($deal) && $deal['CATEGORY_ID'] != $profile['options']['deal_category']) {
			$has_conformity = false;
		}
		// Check order filter
		$filter = isset($profile['filter']['filter']) ? (array)$profile['filter']['filter'] : [];
		foreach ($filter as $item) {
			if (in_array($item['field'], $excludes)) {
				continue;
			}
			// Order site
			if ($item['field'] == 'site') {
			...
			elseif ($item['field'] == 'pay_type') {
				$has_conformity = self::checkFilterConformity($has_conformity, $item, $order_data['PAY_TYPE_ID']);
			}
			// Delivery type
			...
			elseif ($item['field'] == 'user_group') {
				$has_conformity = self::checkFilterConformity($has_conformity, $item, $order_data['USER_GROUPS_IDS']);
			}
			// Order type
	 ...
	 * @return false|void
	 */
	public static function getOrderProfile(array $order_data, array $deal = []) {
		$profile = ProfilesTable::findProfile($order_data, $deal);
		\SProdIntegration::Log('(getOrderProfile) selected profile "' . $profile['id'] . '"');
		if ( ! $profile && ! empty($deal)) {
			$profile = ProfilesTable::getProfileByOrder($order_data, $deal);
			\SProdIntegration::Log('(getOrderProfile) selected profile "' . $profile['id'] . '"');
		}
	...
	/**
	 * Find profile by deal data
	 */
	public static function getProfileByDeal($deal) {
		$result = false;
		...
		if (!empty($list)) {
			foreach ($list as $profile) {
				if ($profile['neworder']['active']) {
					// Check profile by deals filter
					if (self::checkProfileByDeal($deal, $profile)) {
						$result = $profile;
						break;
					}
					// TODO Check profile by orders filter
				}
			}
		}
		return $result;
	...
	public static function checkProfileByDeal($deal, $profile, $excludes=[]) {
		$has_conformity = true;
		// Check deal category
		if (!empty($deal) && $deal['CATEGORY_ID'] != $profile['options']['deal_category']) {
			$has_conformity = false;
		...
		// Check order filter
		$filter = (array)$profile['neworder']['filter'];
		foreach ($filter as $item) {
			if (in_array($item['field'], $excludes)) {
				continue;
			}
			...
			elseif ($item['field'] == 'source') {
				$has_conformity = self::checkFilterConformity($has_conformity, $item, $deal['SOURCE_ID']);
			}
			// Order type
			elseif (strpos($item['field'], 'field_') !== false) {
				$field_id = str_replace('field_', '', $item['field']);
				$has_conformity = self::checkFilterConformity($has_conformity, $item, $deal[$field_id]);
			}
		}
		return $has_conformity;
	}
