...
$APPLICATION->SetTitle(Loc::getMessage('SP_CI_FBASKET_SETTINGS_TITLE'));

CUtil::InitJSCore(array("jquery"));
Asset::getInstance()->addString('<script>
function iframeResize() {
    var iFrameID = document.getElementById(\'sprod_integr_fbasket_settings_frame\');
    if (iFrameID) {
        var cont_h = (iFrameID.contentWindow.document.body.scrollHeight + 30) + \'px\';
        $(\'#sprod_integr_fbasket_settings_frame\').height(cont_h);
    }
}
</script>');
...
?>
    <iframe src="sprod_integr_fbasket_settings_page.php" class="sprod-integr-frame" id="sprod_integr_fbasket_settings_frame"></iframe>
<?
